...
if gpus:
    try:
        ...
        print("TensorFlow is limited to 2 GB memory on the first GPU.")
...
def load_images_cv(path):
    data_list = list()
    for filename in os.listdir(path):
        # Load the image
        image = cv2.imread(path + filename)
        resized_image = np.reshape(image, (64, 64, 3))
        # resized_image = tf.reshape(resized_image, (-1, 64, 64, 3))
...
print('Loading data...')
dataA_all = load_images(path + 'ACGPN_TrainData/train_color/')
...
dataB_all = load_images(path + 'ACGPN_TrainData/train_edge/')
...
print('data A length : ', len(data[0]))
print('data B length : ', len(data[1]))
